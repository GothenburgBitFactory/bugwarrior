...
# This maps service names (defined by the ServiceConfig.service attribute) to
# the prefix which was formerly defined in the ServiceConfigMetaclass `prefix`
# parameter.
PREFIX_MAP = {
    'activecollab': 'activecollab',
    'activecollab2': 'activecollab2',
    'azuredevops': 'ado',
    'bitbucket': 'bitbucket',
    'bts': 'bts',
    'bugzilla': 'bugzilla',
    ...
    'taiga': 'taiga',
    'teamlab': 'teamlab',
    'teamwork_projects': 'teamwork_projects',
    'trac': 'trac',
    'trello': 'trello',
        ...
        if isinstance(name, str):
            if name == 'general' or re.match(r'^flavor\.', name):
                ...
                for key in section.keys():
                    prefix = PREFIX_MAP[section['service']]
                ...
                for k in INTEGERS.get(section['service'], []):
                    to_int(section, k)
                for k in CONFIGLIST.get(section['service'], []):
                    to_list(section, k)
                for k in BOOLS.get(section['service'], []):
                    to_bool(section, k)
