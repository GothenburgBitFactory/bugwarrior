...
def synchronize(
    ...
    for issue in issue_generator:
        ...
        if issue.identifier in issue_map:
            ...
            issue_map[issue.identifier].task.tags = sorted(
                set(issue_map[issue.identifier].task.tags) | set(issue.task.tags)
            )
    ...
    for collected in issue_map.values():
        ...
        # Flatten the typed Task to a taskwarrior record only now, at the sync
        # boundary where taskw needs a plain dict.
...
def build_uda_config_overrides(services: Iterable[str]) -> dict[str, Any]:
    ...
    for service in services:
        targets_udas.update(get_service(service).UDAS_CLASS.get_udas())
